...
export const downloadFontZip = async (css: string, fontSrcPrefix: string) => {
    ...
    for (const lineRaw of lines) {
        ...
        if (line.startsWith("font-style:")) {
            fonStyle =
                line
                    ?.match(fontFaceFontStyleRegex)?.[0]
                    ?.replace("font-style:", "")
                    ?.trim() || "";
        }
        if (line.startsWith("font-weight:")) {
            fontWeight =
                line
                    ?.match(fontFaceFontWeightRegex)?.[0]
                    ?.replace("font-weight:", "")
                    ?.trim() || "";
        }
        if (line.startsWith("font-family:")) {
            frontFamily =
                line
                    ?.match(fontFaceFontFamilyRegex)?.[0]
                    ?.replace("font-family:", "")
                    ?.trim() || "";
        }
        if (line.startsWith("src:")) {
            ...
            if (src && format && frontFamily && fonStyle && fontWeight) {
                linksToDownload.push({
                    src,
                    fileName:
                        `${frontFamily}-${characterSet}-${fonStyle}-${fontWeight}.${format}`
                            .replace(/['"]/gm, "")
                            .toLowerCase(),
                });
            }
    ...
    console.log("🚀[downloadFont.helper.ts:57]: responses: ", responses);
};
